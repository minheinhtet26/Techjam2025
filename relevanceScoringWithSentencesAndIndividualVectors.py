...
for type in types_in_array:
    print(f"\n\nWord: {type}")
    score = 0
    avg_score = 0
    word_vec = nlp(type).vector.reshape(1, -1)

    for sent in doc.sents:
        sim = cosine_similarity(word_vec, sent.vector.reshape(1, -1))[0][0]
        print(f"Similarity({type}, {sent.text}) = {sim:.4f}")
        score = score + sim
...
print("\nAverage Relevance Score is " + str(total_score/len(types_in_array)))